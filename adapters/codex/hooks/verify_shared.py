...
here so they cannot drift. The prefixes are codex-* (not claude-code-*) so
both harnesses can coexist on one machine with honest, debuggable per-harness
/tmp markers.

Codex payload notes: session_id and cwd are documented common stdin keys, so
...
TESTS_CHANGE_PREFIX = "codex-changed-tests"
BUILD_CHANGE_PREFIX = "codex-changed-build"
REVIEW_CHANGE_PREFIX = "codex-changed-review"
...
def session_marker(prefix, session_id):
    ...
    the armed marker to be found. If a codex subagent's PostToolUse fires with
    its own session_id rather than the main session's, the reader looks at a
    different path and the gate silently no-ops for that Stop (RISK-8 - the same
...
def run_verify_gate(detect_cmd, *, retry_prefix, change_prefix, fail_message, timeout):
    ...
    # Avoid re-blocking inside an already-blocked Stop loop (RISK-5: absent on
    # codex reads as falsy and the retry cap below is the structural loop-breaker).
