"""Tests for hooks/block-git-writes.sh - the codex PreToolUse shell guard.

CONTRACT (codex denial channel - NOT the Claude stdout-JSON shape):
  - Reads {"tool_name":"<shell tool>","tool_input":{"command":"<cmd>"}} on stdin.
  - ALLOW: exit 0 with no denial (stderr carries no block reason).
  - BLOCK: exit 2 with the reason written to stderr - the survey's documented
    codex blocking mechanism ("Exit code 2: indicates blocking/denial with
    reason written to stderr").
  - A tool_name outside the shell matcher set
    (shell|local_shell|bash|Bash|exec_command) exits 0 immediately (ALLOW).
  - RISK-3 tolerant reads: tool_input.command may be a string or an argv
    array (joined); tool_input.cmd is the last-resort string field.

The segment-matching corpus (TC-01..TC-108 and the TC-derived-* families) is
ported verbatim from the claude-code adapter - the guard LOGIC is identical;
only the I/O contract differs. See the claude-code suite's module docstring
for the full history of each case range.
"""
...
def run_guard(command=None, tool_name="Bash", env=None):
    ...
    Builds the stdin JSON and returns the CompletedProcess (the codex contract
    is judged on returncode + stderr, not stdout).
    For non-shell tool_names, command may be omitted (None).
    """
    tool_input = {}
...
# expected_block_bool=True  -> exit 2 with a non-empty reason on stderr (BLOCK)
# expected_block_bool=False -> exit 0 (ALLOW)
...
# Codex-specific I/O cases: the shell tool may appear under any of the matched
# names (RISK-2); Read stays outside the matcher set and is always allowed.
CASES += [
    ("TC-codex-shell-block", "shell", "git reset --hard", True),
    ("TC-codex-shell-allow", "shell", "git add -A", False),
    ("TC-codex-local-shell-block", "local_shell", "git push --force", True),
    ("TC-codex-bash-lower-block", "bash", "git rebase -i HEAD~3", True),
    ("TC-codex-exec-command-block", "exec_command", "git clean -fd", True),
]
...
def test_block_git_writes(case_id, tool_name, command, expected_block):
    ...
    against the codex denial channel (exit 2 + stderr reason)."""
    result = run_guard(command=command, tool_name=tool_name)
    if expected_block:
        assert result.returncode == 2, (
            f"{case_id}: expected BLOCK (exit 2) for tool_name={tool_name!r} "
            f"command={command!r}; got returncode={result.returncode}; "
            f"stdout={result.stdout!r} stderr={result.stderr!r}"
        )
        assert result.stderr.strip() != "", (
            f"{case_id}: a BLOCK must write its reason to stderr; "
            f"got stderr={result.stderr!r}"
        )
        assert "blocked" in result.stderr, (
            f"{case_id}: the stderr reason must name the block; "
            f"got stderr={result.stderr!r}"
        )
    else:
        assert result.returncode == 0, (
            f"{case_id}: expected ALLOW (exit 0) for tool_name={tool_name!r} "
            f"command={command!r}; got returncode={result.returncode}; "
...
# ---------------------------------------------------------------------------
# RISK-3 tolerant payload reads: argv-array command and the cmd fallback field
# ---------------------------------------------------------------------------


def _run_raw(payload_dict, env=None):
    return subprocess.run(
        [BASH_PATH, str(HOOK_PATH)],
...
def test_argv_array_command_blocks():
    """tool_input.command as an argv array is joined and judged like a string."""
    result = _run_raw(
        {"tool_name": "shell", "tool_input": {"command": ["git", "reset", "--hard"]}}
    )
    assert result.returncode == 2, (
        f"argv-array destructive command must block (exit 2); "
        f"got {result.returncode}; stderr={result.stderr!r}"
    )
    assert "blocked" in result.stderr, f"got stderr={result.stderr!r}"


def test_argv_array_safe_command_allows():
    result = _run_raw(
        {"tool_name": "shell", "tool_input": {"command": ["git", "add", "-A"]}}
    )
    assert result.returncode == 0, (
        f"argv-array safe command must pass (exit 0); "
        f"got {result.returncode}; stderr={result.stderr!r}"
    )


def test_cmd_fallback_field_blocks():
    """tool_input.cmd is read when tool_input.command is absent."""
    result = _run_raw(
        {"tool_name": "shell", "tool_input": {"cmd": "git push --force"}}
    )
    assert result.returncode == 2, (
        f"tool_input.cmd destructive command must block (exit 2); "
        f"got {result.returncode}; stderr={result.stderr!r}"
    )


def test_empty_tool_input_allows():
    """No derivable command at all is an allow (nothing to judge)."""
    result = _run_raw({"tool_name": "shell", "tool_input": {}})
    assert result.returncode == 0, (
        f"got {result.returncode}; stderr={result.stderr!r}"
    )
...
# ---------------------------------------------------------------------------
# jq-absent fail-open / jq-present fail-closed
# ---------------------------------------------------------------------------


def test_jq_absent_fail_open(tmp_path):
    """No jq on PATH: exit 0, no denial, and a jq warning on stderr."""
...
def test_jq_present_parse_failure_stays_fail_closed():
    """jq present but the hook payload is malformed JSON: stays fail-closed -
    exit 2 with the parse-failure reason on stderr (the codex denial channel).

    Negative control for the jq-absent fail-open: fail-open covers only the
    "jq missing" case. A genuinely unparseable payload with jq present and
    working is a different anomaly and must still deny.
    """
    result = subprocess.run(
        [BASH_PATH, str(HOOK_PATH)],
    ...
    assert result.returncode == 2, (
        f"expected exit 2 (fail-closed denial) on an unparseable payload; "
        f"got {result.returncode}; stdout={result.stdout!r} stderr={result.stderr!r}"
    )
    assert result.stderr.strip() != "", (
        f"the fail-closed denial must carry its reason on stderr; "
        f"got stderr={result.stderr!r}"
    )
