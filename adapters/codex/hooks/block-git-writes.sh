#!/usr/bin/env bash
# PreToolUse hook (codex port): block the dangerous, allow the forward.
# Parses shell tool input, splits the command into pipeline segments, and blocks
...
# passes. Logs blocked attempts to ~/.codex/debug/git-block.log (rotated at ~1MB).
#
# Codex denial channel: exit 2 with the reason on stderr - the one blocking
# mechanism the codex survey documents verbatim ("Exit code 2: indicates
# blocking/denial with reason written to stderr"). Allow = exit 0, no output.

set -euo pipefail

log_file="$HOME/.codex/debug/git-block.log"
mkdir -p "$HOME/.codex/debug" 2>/dev/null || true
...
# Deny without relying on jq - used on the fail-closed jq-parse-failure paths
# below (jq present but the payload unparseable is a genuine anomaly). Reason
# goes to stderr, exit 2 (codex denial channel).
block_static() {
  local reason="$1"
...
# Fail open if jq is missing: warn and allow. The prior fail-closed choice
# turned a missing jq into a total block of every shell call.
...
# Only evaluate shell tool calls. RISK-2: codex's shell tool name is
# undocumented - match the plausible spellings broadly.
if [[ ! $tool_name =~ ^(shell|local_shell|bash|Bash|exec_command)$ ]]; then
  exit 0
fi

# Tolerant command read (RISK-3): tool_input.command as string, else as an
# argv array (joined), else tool_input.cmd.
if ! command=$(echo "$input" | jq -r '
    if (.tool_input.command | type) == "string" then .tool_input.command
    elif (.tool_input.command | type) == "array" then (.tool_input.command | map(tostring) | join(" "))
    elif (.tool_input.cmd | type) == "string" then .tool_input.cmd
    else empty end' 2>/dev/null); then
  block_static "git-write guard could not parse the shell command from hook input. Run the command yourself or report this."
fi
...
while IFS= read -r seg; do
  ...
  if [[ $seg =~ $blocked_all ]]; then
    log "BLOCKED: $command"
    printf '%s\n' "This git command rewrites or destroys history/state and is user-only. Forward ops (add/commit/push) are allowed; this one is blocked: ${command}. If you explicitly want it, surface the exact command for the user to run." >&2
    exit 2
  fi
done <<< "$segments"
