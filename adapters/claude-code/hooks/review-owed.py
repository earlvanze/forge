...
written by review *subagents*, usually via Bash (the Write tool blocks
subagents from "report files") and always under a subagent session_id - so a
PostToolUse clearer neither fires nor rendezvous with the main session's
marker. This Stop gate runs under the main session_id and reads the files off
disk, so it is agnostic to both how the findings were written and by whom. The
